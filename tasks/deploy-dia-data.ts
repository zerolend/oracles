...
task(`deploy-dia-data`)
  ...
  .addParam("defaultKey")
  .setAction(async ({ consumeraddress, vaultAddress, defaultKey }, hre) => {
    ...
    const contract = await deployments.deploy("DiaDataAggregator", {
      ...
      args: [consumeraddress, vaultAddress, defaultKey],
    ...
    if (process.env.NODE_ENV != "test") {
      await hre.run("verify:verify", {
        ...
        constructorArguments: [consumeraddress, vaultAddress, defaultKey],
      });
