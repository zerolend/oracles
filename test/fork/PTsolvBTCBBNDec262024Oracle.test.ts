...
describe("PTsolvBTCBBNDec262024Oracle Fork Test", function () {
  ...
  beforeEach(async function () {
    ...
    const MorphoOracle = await ethers.getContractFactory(
      "PTsolvBTCBBNDec262024Oracle"
    );
    oracle = await MorphoOracle.deploy();
    await oracle.waitForDeployment();
  });

  it("should return the correct solvBTC/USD price with 8 decimals", async function () {
    const usdPrice = await oracle.usdPrice();
    const rawPrice = await oracle.rawPrice();
    const latestAnswer = await oracle.latestAnswer();

    console.log("PT-solvBTC/solvBTC Price:", Number(rawPrice) / 1e18);
    console.log("solvBTC/USD Price:", Number(usdPrice) / 1e8);
    console.log(
      "PT-solvBTC/USD Price:",
      (Number(latestAnswer) / 1e8).toString()
    );
  ...
  it("should return solvBTC/USD price after expiry", async function () {
